...
void setup() {
  ...
  Serial.println("Calibrating MQ-7 in clean air...");
...
void loop() {
  ...
  if (ppm >= 10 && ppm < 50) {
    Serial.println("Low CO concentration — mild caution");
  }
  else if (ppm >= 50 && ppm < 400) {
    Serial.println("Carbon Monoxide(CO)");
  }
  else if (ppm >= 400 && ppm < 1000) {
    Serial.println("Carbon Monoxide (CO); Toxic CO concentrated");
  } 
  else if (ppm >= 1000) {
    Serial.println("Carbon Monoxide (CO); Toxic CO concentrated; High CO level");
  }
