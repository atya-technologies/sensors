...
void setup() {
  ...
  Serial.println("Calibrating MQ-8 in clean air...");
...
void loop() {
  ...
  if (ppm >= 100 && ppm < 500) {
    Serial.println("Low Hydrogen presence");
  }
  else if (ppm >= 500 && ppm < 800) {
    Serial.println("Hydrogen (H₂)");
  }
  else if (ppm >= 800 && ppm < 1000) {
    Serial.println("Hydrogen(H₂); LOW Methane (CH₄)");
  } 
  else if (ppm >= 1000) {
    Serial.println("Hydrogen(H₂); LOW Methane (CH₄); Flammable gas");
  }
