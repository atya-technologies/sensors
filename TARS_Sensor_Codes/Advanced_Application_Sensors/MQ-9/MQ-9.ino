...
void loop() {
  ...
  if (ppm >= 20 && ppm < 500) {
    Serial.println("Low Carbon Monoxide(CO)");
  }
  else if (ppm >= 500 && ppm < 800) {
    Serial.println("Carbon Monoxide(CO)");
  }
  else if (ppm >= 800 && ppm < 1000) {
    Serial.println("Carbon Monoxide(CO); Moderate Methane (CH₄)");
  } 
  else if (ppm >= 1000) {
    Serial.println("Carbon Monoxide(CO); Moderate Methane (CH₄); LPG (Propane/Butane)");
  }
